...
public class RedisUtil {
	 ...
	 * @return true 存在 false不存在
	 */
	public boolean zHasKey(String key, String value) {
		try {
	 ...
	 * @return
	 */
	public Set<String> zRangeReverse(String key, int start, int end) {
		try {
	 ...
	 * @return
	 */
	public Set<String> zRange(String key, int limit, int size) {
		try {
			return redisTemplate.opsForZSet().range(key, limit, size);
		} catch (Exception e) {
			LOGGER.error(ExceptionUtil.stacktraceToString(e));
			return Collections.emptySet();
		}
	 ...
	 * @return
	 */
	public double zIncr(String key, String value, double inc) {
		try {
	...
	//------------------------------执行lua脚本------------------------------
	public Object execute(String luaScript, List<String> keys, String... args) {

		// 脚本sha1值
		String luaScriptSha1 = SecureUtil.sha1(luaScript);

		// 脚本
		RedisScript<Object> redisScript = new RedisScript<Object>() {
			@Override
			public String getSha1() {
				return luaScriptSha1;
			}
